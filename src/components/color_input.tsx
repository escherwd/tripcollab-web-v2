...
export default function ColorInput({
  ...
  const initialOtherColor =
    initialColor && !colors.includes(initialColor) ? initialColor : "#7c3aed";

  const [contrastingOtherColor, setContrastingOtherColor] =
    useState<string>("#fff");
  ...
  return (
    <div className={`flex gap-1 justify-start items-center ${viewOnly ? 'cursor-not-allowed! *:pointer-events-none' : ''}`}>
      ...
      <div
        className={`relative cursor-pointer border-2 border-gray-100 rounded-full size-8 overflow-hidden ${isOtherColorSelected ? "ring-2 ring-black" : ""}`}
      >
        <input
          type="color"
          defaultValue={initialOtherColor}
          className="size-12 absolute -inset-2 cursor-pointer"
          onChange={onColorSelect}
        />
        <EllipsisHorizontalIcon
          className="size-4 absolute inset-1.5 pointer-events-none cursor-pointer transition-colors"
          style={{ color: contrastingOtherColor }}
        />
      </div>
      <div className="flex-grow" />
      <PanelIconButton disabled={viewOnly} icon={<XMarkIcon />} onClick={() => updateColor(null)} />
    </div>
  );
}
