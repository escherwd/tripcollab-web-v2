...
export const serverUpdateUser = async (
  ...
  if (changes.username && changes.username.length < 3) throw "Username must be 3+ characters"
  if (changes.username && changes.username.length > 24) throw "Username must be less than 24 characters"

  if (changes.firstName && changes.firstName.length > 32) throw "Display name must be less than 32 characters"

  if (changes.location && changes.location.length > 64) throw "Location must be less than 64 characters"
  if (changes.website && changes.website.length > 128) throw "Website must be less than 128 characters"
  if (changes.website && !changes.website.startsWith('https://')) throw "Websites must be prepended with https://"
